...
class ForecastIO{
  ...
  function getHistoricalConditions($latitude, $longitude, $timestamp) {
    ...
    $exclusions = 'currently,minutely,alerts';
    ...
    if ($data !== false) {
        ...
        $averageWS = 0;
        $count = 0;
        foreach($data->hourly->data as $hdata){
            $averageTemp += $hdata->temperature;
            $averageWS += $hdata->windSpeed;
            $count++;
        }
        $averageTemp = $averageTemp/$count;
        $averageWS = $averageWS/$count;
        
        $conditions  = new ForecastIOConditions($data->daily->data[0]);
        $conditions->setAverageTemperature($averageTemp);
        $conditions->setAverageWindSpeed($averageWS);
        
        $conditions->setUnits($data->flags->units);
...
class ForecastIOConditions{
  ...
  function setAverageTemperature($temp) {
        $this->raw_data->averageTemperature = $temp;
  }
  
  function setAverageWindSpeed($ws) {
        $this->raw_data->averageWindSpeed = $ws;
  }
